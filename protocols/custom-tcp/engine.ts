...
/** Configuration for the Custom TCP engine. */
export interface CustomTcpConfig {
  host: string;
  port: number;
...
/**
 * Custom TCP protocol engine with HEX/ASCII/raw send modes, timed auto-send,
 ...
 */
export class CustomTcpEngine extends AdapterBackedDriver<CustomTcpConfig, CustomTcpRuntime> {
  protected protocolId = "custom-tcp";

  protected defaultConfig: CustomTcpConfig = {
    host: "127.0.0.1",
    port: 8080,
  ...
  protected buildAdapter(config: CustomTcpConfig): ProtocolAdapter<CustomTcpRuntime, CustomTcpConfig> {
    const self = this;
    return {
      ...
      createReadPlan: (request: ReadRequest, context) => {
        ...
        return {
          id: `custom-tcp-read-${Date.now()}`,
          type: "custom-tcp-read",
          stages: [
            {
              id: "send-and-await",
      ...
      createWritePlan: (request: WriteRequest, context) => {
        return {
          id: `custom-tcp-write-${Date.now()}`,
          type: "custom-tcp-write",
          stages: [
            {
              id: "send-raw",
              send: () => self.resolveSendData(request, context.config),
   ...
   * @example
   * ```ts
   * const engine = new CustomTcpEngine();
...
export default CustomTcpEngine;
